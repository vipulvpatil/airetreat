...
const autoJoinGame = (userEmail, playerId) => {
  const client = grpcClient()
  const autoJoinGameRequest = {playerId}

  return new Promise((resolve, reject) => {
    client.AutoJoinGame(autoJoinGameRequest, metadataWithRequestingUserEmail(userEmail), (err, autoJoinGameResponse) => {
      if (err) {
        handleGrpcError(reject, err)
      } else {
        resolve(autoJoinGameResponse)
      }
    })
  })
}
