...
const sendMessage = (gameId, playerId, botId, text, type) => {
  const client = grpcClient()
...
const tag = (gameId, playerId, botId) => {
  const client = grpcClient()
  const sendMessageRequest = {gameId, playerId, botId}
  var meta = new Metadata()

  return new Promise((resolve, reject) => {
    client.Tag(sendMessageRequest, meta, (err, sendMessageResponse) => {
      if (err) {
        reject(err)
      } else {
        resolve(sendMessageResponse)
...
const help = (gameId, playerId) => {
  const client = grpcClient()
  const helpRequest = {gameId, playerId}
  var meta = new Metadata()

  return new Promise((resolve, reject) => {
    client.help(helpRequest, meta, (err, helpResponse) => {
      if (err) {
        reject(err)
      } else {
